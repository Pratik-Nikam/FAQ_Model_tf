...
# Step 4: Evaluate the model and calculate accuracy
...
def calculate_accuracy(true_labels, predicted_labels):
    ...
    for true, pred in zip(true_labels, predicted_labels):
        for label in true:
            if true[label] == 1 and pred[label] >= 0.5:  # Threshold for binary classification
                correct += 1
            elif true[label] == 0 and pred[label] < 0.5:
                correct += 1
            total += 1
...
def main():
    ...
    # Step 5: Evaluate the model and calculate accuracy
